...
class LoadData(Dataset):
    ...
    def __init__(self, path='data/ecoli_100_space_fix.csv', split_r=0.9, is_train=True, gpu_ids='0', num=500):
        realB = list(pd.read_csv(path)['realB'].head(-num))
        realA = list(pd.read_csv(path)['realA'].head(-num))
        realA = [sublist[-num:] if len(sublist) >= num else sublist for sublist in realA]
        realB = [sublist[-num:] if len(sublist) >= num else sublist for sublist in realB]
