...
class LoadData(Dataset):
    ...
    def __init__(self, path='data/ecoli_100_space_fix.csv', split_r=0.9, is_train=True, gpu_ids='0', num=500):
        realB = list(pd.read_csv(path)['realB'])#[:50]
        realA = list(pd.read_csv(path)['realA'])#[:50]
        realA = [sublist for sublist in realA if len(sublist)==num] 
        realB = [sublist for sublist in realB if len(sublist)==num]
