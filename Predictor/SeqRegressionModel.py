...
from utils.peft_modules import *
...
def Seq2Scalar(input_nc,
    ...
    if mode == 'Hanwen':
        ...
        return DenseNet(input_nc=5, growth_rate=32, block_config=(2, 2, 4, 2),
        ...
        return DenseConnectedLSTM(input_nc=5, growth_rate=32, block_config=(2, 2, 4, 2),
                 num_init_features=64, bn_size=4, drop_rate=0.2, input_length=seqL)
...
class DenseLSTM(nn.Module):
    ...
    def __init__(self, input_nc=4, growth_rate=32, block_config=(2, 2, 4, 2),
        ...
        #Linear layer
        dim_num = 30
        self.ratio = nn.Linear(int(length) * num_features, 128)
        self.prob = nn.Linear(128,dim_num)
        # self.scale = nn.Parameter(torch.tensor(1., requires_grad=True))
        # self.bias = nn.Parameter(torch.tensor(1., requires_grad=True))
        self.scale = nn.Parameter(torch.full((1, dim_num), 1.0)) 
        self.bias = nn.Parameter(torch.full((1, dim_num), 0.1)) 
        
        #self.ratio = nn.Linear(int(length) * num_features, 1)
        #self.dropout1 = nn.Dropout2d(p=0.1)  # 用于卷积层后的二维Dropout
        self.dropout2 = nn.Dropout(p=0.3) # 0.3
        self.adapter_former = AdaptFormer(int(length) * num_features,4)
    ...
    def forward(self, x):
        ...
        #adpf
        out = self.adapter_former(out) + out
        
        l2_norm = torch.norm(self.ratio.weight, p=2, dim=1, keepdim=True)
        self.ratio.weight = nn.Parameter(self.ratio.weight / l2_norm)
        out = self.ratio(out)
        out = F.relu(out)
        #out = self.dropout2(out)        
        l2_norm = torch.norm(self.prob.weight, p=2, dim=1, keepdim=True)
        self.prob.weight = nn.Parameter(self.prob.weight / l2_norm)
        out = self.prob(out)
