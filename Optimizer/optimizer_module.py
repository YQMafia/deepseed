...
class optimizer_fix_flank:
    ...
    def __init__(self,
        ...
        self.generator = torch.load(generator_path, map_location=torch.device('cpu'))
        self.predictor = torch.load(predictor_path, map_location=torch.device('cpu'))
        self.is_gpu = is_gpu
        self.seqL = seqL
    ...
    def set_input(self, polish_seqs, control_seqs):
        ...
        for i in range(len(polish_seqs)):
            ...
            mask_i = np.zeros([5, len(seq_i)])
            for j in range(len(seq_i)):
                if seq_i[j] == 'M':
                    mask_i[:, j] = np.ones([5, 1])[:, 0]
    ...
    def opt_func(self, p):
        ...
        p_reshape = np.zeros([np.size(p, 0), 5, self.seqL])
        mask_i = self.masks[self.i]
        for i in range(np.size(p, 0)):
            p_reshape[i, :, :] = np.multiply(self.seqs[self.i], 1 - self.masks[self.i]) + np.multiply(mask_i, p[i, :].reshape([5, -1]))
        with torch.no_grad():
            positionData = DataLoader(LoadData(data=p_reshape), batch_size=512, shuffle=False)
            tensorSeq, pred_value = [], []
            for j, eval_data in enumerate(positionData):
                tensorSeq.append(self.generator(eval_data.cpu()).detach())
            ...
            generateData = DataLoader(LoadData(data=tensorSeq), batch_size=512, shuffle=False)
            predictions = []
            seq_generate = []
            print('evaluate predictor')
            for j, eval_data in tqdm(enumerate(generateData)):
                seq_generate.append(eval_data)
                predictions.append(self.predictor(eval_data.cpu()).detach())
            seq_generate = torch.cat(seq_generate, dim=0).cpu().float().numpy()

            predictions = torch.stack(predictions, dim=0).cpu().float().squeeze().numpy()
            try:
                for k in range(len(predictions)):
                    seq_decode_k = decode_oneHot(np.squeeze(seq_generate[k, :, :]).reshape([5, -1]))
                ...
                seq_max = seq_generate[0]
                expression_eval = predictions
                print(predictions.dtype, expression_eval)
    ...
    def optimization(self):
        ...
        for i in tqdm(range(len(self.seqs))):
            ...
            self.i = i
            lb, ub = [], []
            for j in range(5*self.seqL):
                lb.append(0)
                ub.append(1)
            
            torch.cuda.is_available = lambda: False
            
            ga = GA(func=self.opt_func, n_dim=5*self.seqL, size_pop=self.size_pop, max_iter=self.max_iter, prob_mut=self.prob_mut, lb=lb, ub=ub,
                    precision=1e-7)
            ga.run()
            print('Optimize Expression: {}'.format(2 ** self.expr_results[seq_i]))
